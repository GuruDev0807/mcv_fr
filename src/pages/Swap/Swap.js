...
const Swap = () => {
  ...
  const getTokenInfo = async (
    ...
    try {
      ...
      const quoteFromAhill = await swapContract.quote(
        +Math.floor(ethers.utils.formatEther(ahillBalance)),
        0
      );
      const quoteFromGville = await swapContract.quote(
        +Math.floor(ethers.utils.formatEther(gvilleBalance)),
        1
      );
  ...
  const handleSwap = useCallback(async () => {
    ...
    try {
      ...
      if (name === "AHILL") {
        const ahillAllowance = await ahillContract.allowance(
          address,
          swapAddress
        );
        if (parseInt(ethers.utils.formatEther(ahillAllowance)) < 1) {
          const res = await ahillContract.approve(
            swapAddress,
      ...
      if (name === "GVILLE") {
        const gvilleAllowance = await gvilleContract.allowance(
          address,
          swapAddress
        );
        if (parseInt(ethers.utils.formatEther(gvilleAllowance)) < 1) {
          const res = await gvilleContract.approve(
            swapAddress,
            ethers.utils.parseEther("99999999999999")
          );
          await res.wait();
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(180deg, rgba(0,48,74,0.8) 0%, rgba(0,0,0,0) 100%)",
        backgroundBlendMode: "multiply",
      }}
    >
      <div className="text-[#FFCE0D]  text-xs leading-4 tracking-[0.5px]">
        This is where you can get an estimate and swap your GVILLE or AHILL into
        the new MCV token.
      </div>
      <div className="my-4 border-t border-white border-dashed"></div>
      <div className="flex items-center gap-1.5 p-2">
        <img src={iconGville} alt="token_gville" className="w-5" />
        <div className="text-white  text-xs leading-3 tracking-[2px] uppercase text-shadow-blue">
          gville balance:
        </div>
        <div className="text-[#00CAFF]  text-lg leading-3 tracking-[2px] font-bold text-shadow-blue">
          {gvilleBalance.toLocaleString()}
        </div>
      </div>
      <div className="flex items-center gap-1.5 p-2">
        ...
        <div className="text-[#00CAFF]  text-lg leading-3 tracking-[2px] font-bold text-shadow-blue">
          {ahillBalance.toLocaleString()}
        </div>
      </div>
      <div className="my-2 p-2 bg-[#022133] bg-opacity-60 border border-[#2D4756] rounded-xl">
        <div className="flex items-center justify-center mt-2">
          <div
            className="text-[#33D4FF]  font-bold text-xl leading-4 tracking-[2.7px] uppercase"
            style={{ textShadow: "0px 0px 5px rgba(50, 171, 252, 0.98)" }}
          >
            swap
          </div>
          <div className="w-px border border-r border-dashed border-[#FFD900] h-5 rotate-[30deg] mx-4"></div>
          <div className="text-white  text-xs leading-4 tracking-[2px] uppercase text-shadow-blue">
            swap into mcv
          </div>
        </div>
        <div className="my-4 border-t border-white border-dashed"></div>
        <div className="flex justify-between px-2">
          <div>
            <div className="text-[#00CAFF]  text-xs leading-3 tracking-[1px] uppercase text-shadow-blue">
              you have:
            </div>
            <div className="text-white  text-2xl leading-4 tracking-[1px] mt-2 text-shadow-blue">
              {selectedToken.name === "GVILLE"
                ? gvilleBalance.toLocaleString()
                : ahillBalance.toLocaleString()}
            </div>
          </div>
          <div>
            <Listbox value={selectedToken} onChange={setSelectedToken}>
              <div className="relative mt-1 min-w-[110px]">
                <Listbox.Button className="relative flex items-center gap-1.5 w-full py-2 pl-3 pr-5 bg-[#00A3FF] bg-opacity-5 border border-[#BDF1FF] rounded-full cursor-default focus:outline-none  focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 sm:text-xs">
                  <span>
                    <img
                ...
                <Transition
                  ...
                  leaveTo="opacity-0"
                >
                  <Listbox.Options className="absolute w-full py-1 mt-1 overflow-auto text-xs text-white tracking-[2px] bg-[#183950] border border-[#BDF1FF] rounded-lg shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-[10px]">
                    {tokens.map((token, tokenIdx) => (
                      <Listbox.Option
                        key={tokenIdx}
                        className={({ active }) =>
                          `relative cursor-default select-none py-2 pl-10 pr-4 ${
                            active ? "bg-cyan-100 text-cyan-900" : "text-white"
                          }`
                        }
                        value={token}
                      >
        ...
        <div className="my-4 border-t border-[#FFD900] border-dashed"></div>
        <div className="flex justify-between px-2">
          <div>
            <div className="text-[#00CAFF]  text-xs leading-3 tracking-[1px] uppercase text-shadow-blue">
              you will receive:
          ...
          <div>
            ...
            <div className="float-right">
              <div className="flex items-center gap-2 mt-2">
                ...
                <div className="text-[#B3DAE0]  text-xs leading-3 tracking-[2px] uppercase text-shadow-blue">
                  mcv
                </div>
              </div>
            </div>
        ...
        <div className="mt-2">
          <button
            className="w-full py-2 bg-[#00CAFF] hover:bg-cyan-500 rounded-lg text-center  text-[#072637] font-bold text-xs tracking-[1px] uppercase"
            onClick={() => handleSwap()}
          >
            swap
          </button>
        ...
        {/* <div className="flex justify-between gap-1 mt-2">
          <div className=" text-[10px] leading-3 text-[#FFCE0D]">
