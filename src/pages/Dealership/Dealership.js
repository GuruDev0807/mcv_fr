...
const Dealership = () => {
  ...
  const { avaxBalance, mcvBalance } = useSelector((state) => state.globalInfo.walletBalance)
  ...
  const init = useCallback(
    async (mintContract, mintWithMcvContract, carContract) => {
      try {
        ...
        console.log(totalMinted.toString(), maxMinted.toString());
  ...
  const handleMint = useCallback(async () => {
    try {
      ...
      if (payMode === "avax") {
        ...
        if (+avaxBalance < 4 * numBoxToBuy) {
          return toast.error("NOT ENOUGH BALANCE");
        }
      }
      if (payMode === "mcv") {
        if (+mcvBalance < 500 * numBoxToBuy) {
          return toast.error("NOT ENOUGH BALANCE");
        }
        if (mcvEnded || totalMintedWithMcv > 999) {
          return toast.error("MINT ENDS");
        }
      }
      setMinting(true);
      ...
      if (payMode === "avax") {
        const price = ethers.utils.parseEther(
          ethers.utils.formatEther(mintPrice) * numBoxToBuy + ""
        );
  ...
  }, [address, avaxBalance, carContract, ended, init, maxMinted, mcvBalance, mcvContract, mcvEnded, mintContract, mintPrice, mintWithMcvContract, minting, numBoxToBuy, payMode, totalMinted, totalMintedWithMcv]);
  ...
  useEffect(() => {
    ...
    const mintWithMcvContract = new ethers.Contract(
      mintWithMcvAddress,
      mintWithTokenABI,
      signer
    );
  ...
  return (
    <>
      <div className="mx-4 md:ml-[13vw] md:mr-0 2xl:mb-4 text-white mt-8">
        <div className="relative pb-4 mb-10 md:pb-40 md:mb-24 2xl:pb-20 2xl:mb-12 callout-bg">
          ...
          <div className="grid gap-2 mt-2 xl:pr-4 xl:grid-cols-2 2xl:grid-cols-7 3xl:grid-cols-3">
            <div className="2xl:col-span-3 3xl:col-span-1 bg-gradient-blue border-[0.5px] border-[#80C9FF] pl-6 pr-4 py-6 md:ml-4 2xl:ml-0 rounded-t-2xl 2xl:rounded-tr-md rounded-b-2xl">
              <img
                src={Images.logoMuscleCarGD}
                alt="logo"
                className="mx-auto md:mx-0"
              />
              <div className="py-5 text-sm leading-5 ">
                Avax Muscle Cars are 10,000 randomly-generated 3D NFTs with
                utility that allows you to play, race, win, and earn crypto.
                AMCC Muscle Cars earn up to 34 MCV per day, and you can race
                your car in the beta game!
              </div>
              ...
              <div className="grid grid-cols-none gap-2 py-2 sm:grid-cols-12">
                <div className="col-span-5">
                  ...
                  <div className="">
                    ...
                    <div className=" text-[10px] leading-3 tracking-[1px] text-white text-shadow-blue py-1 uppercase">
                      - or -
                    </div>
                    <div className="flex items-center gap-1 py-1">
                ...
                <div className="grid grid-cols-2 col-span-7">
                  ...
                  <div>
                    <div className="pb-2">
                      <div className=" text-[10px] leading-3 text-[#EBBB00] tracking-[1px] text-shadow-blue uppercase">
                        minted w/ mcv
                      </div>
                      <div className="text-2xl leading-3 text-whtie tracking-[1px] text-shadow-blue mt-2">
                        {totalMintedWithMcv + 214}
                      </div>
                    </div>
                    <div className="py-2">
                      <div className=" text-[10px] leading-3 text-[#EBBB00] tracking-[1px] text-shadow-blue uppercase">
              ...
              <div className="grid grid-cols-none md:grid-cols-3 xl:grid-cols-none">
                <a
                  className="flex gap-6 py-1 mx-auto md:mx-0"
                  target="_blank"
                ...
                <a
                  className="flex gap-6 py-1 mx-auto md:mx-0"
                  target="_blank"
                  href="https://discord.gg/8vMcCx3R2K"
                  rel="noreferrer"
                >
            ...
            <div className="2xl:col-span-4 3xl:col-span-2 bg-gradient-blue border-[0.5px] border-[#80C9FF] rounded-xl md:ml-4 3xl:ml-0 p-6">
              <div className=" text-lg leading-5 text-[#21D0FF]">
                        ...
                        {index <= numBoxToBuy ? (
                          ...
                          >
                            <img
                              ...
                              alt="icon_ahill"
                              className={`w-full ${
                                index <= hover ? "opacity-80" : "opacity-20"
                              }`}
                              loading="lazy"
                            />
                          </div>
              ...
              <div className=" text-sm leading-5 text-[#FFCE0D] py-0.5">
                {numBoxToBuy} Mints Selected
              </div>
              <div className="border-b border-dashed border-[#2EA0DC] my-2"></div>
              {isMinted ? (
                <div className="py-2 uppercase ">
                  <div className="text-white text-sm tracking-[2.2px] text-shadow-blue">
                    congratulations!
                  </div>
                  <div
                    className="text-[#00E0FF] text-2xl tracking-widest"
                    style={{
                  ...
                  <table className="w-full  text-xs text-white tracking-[2.2px]">
                    ...
                    <tbody className="text-center">
                      {mintedTokenIds.map((tokenId, index) => (
                        <tr
                          className="border-b border-[#80C9FF] border-dashed last:border-0"
                          key={index}
                        >
                          <td className="py-3">
                  ...
                  <div className="flex flex-wrap items-center gap-4 py-2">
                    <div
                      className={`bg-[black] bg-opacity-10 border ${
                        payMode === "avax"
                          ? "border-[#FFCE0D]"
                          : "border-[#547076] hover:border-[#FFCE0D]"
                    ...
                    >
                      ...
                      <div
                        className={` leading-5 ${
                          payMode === "avax" ? "text-[#FFCE0D]" : "text-white"
                        }`}
                      >
                        <div className="text-lg">
                          <span className="font-bold">{numBoxToBuy} Mints</span>{" "}
                          x 4 AVAX
                        </div>
                        <div className="text-sm">
                          You will pay: {numBoxToBuy * 4} AVAX
                        </div>
                      </div>
                    </div>
                    <div
                    ...
                    >
                      ...
                      <div
                        className={` leading-5 ${
                          payMode === "mcv" ? "text-[#FFCE0D]" : "text-white"
                        }`}
                      >
                        <div className="text-lg">
                          <span className="font-bold">{numBoxToBuy} Mints</span>{" "}
                        ...
                        <div className="text-sm">
                          You will pay: {(numBoxToBuy * 500).toLocaleString()}{" "}
                          MCV
                        </div>
                      </div>
                    </div>
                      ...
                      <div>
                        ...
                        <div className="mt-2 text-3xl leading-4 tracking-[1px] text-white uppercase ">
                          <span className="text-shadow-blue">
                            {(
                              numBoxToBuy * (payMode === "avax" ? 4 : 500)
                            ).toLocaleString()}
                          </span>{" "}
      ...
      {revealTokenId && (
        ...
        >
          <div className="flex items-center justify-center min-h-screen text-white">
            <div>
              <div
                className="hidden mb-1 text-yellow-400 cursor-pointer sm:block"
                onClick={() => handleClose()}
              >
                <svg
                  width="32px"
              ...
              <div className="mx-5 sm:mx-0 sm:w-[450px] 2md:w-[900px]">
                <img
                  src={`${imgMuscleCarUrl}${revealTokenId}`}
                  alt=""
                  className="w-full"
                  loading="lazy"
                />
